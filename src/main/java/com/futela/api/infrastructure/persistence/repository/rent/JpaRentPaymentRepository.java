...
public interface JpaRentPaymentRepository extends JpaRepository<RentPaymentEntity, UUID> {
    ...
    @Query("SELECT p FROM RentPaymentEntity p JOIN FETCH p.lease l WHERE l.tenant.id = :tenantId AND p.deletedAt IS NULL")
    List<RentPaymentEntity> findByTenantId(@Param("tenantId") UUID tenantId);
}
